...
SYSCALL_DEFINE1(acquire_resources, int*, ptr){
	int val;
	task_t task;
	...
	int result;
	int result2;
	if(isEmpty(&high) == 0 && isEmpty(&mid) == 0 && isEmpty(&low) == 0 && isEmpty(&waiting) == 0){
		// do nothing
		return 3; // nothing happened
	} else if(isEmpty(&waiting) == 1){
		task = getWaiting(&waiting);
		val = 2;
	} else if(isEmpty(&high) == 1 || isEmpty(&mid) == 1 || isEmpty(&low) == 1){
	...
	if(result == 0 && result2 == 0){
		if(val == 1){
			...
			emptyWaiting(&resources, &waiting, &completed, task);
			return val;
		}
	} else if(result == 0){
		up(&sem2);
	...
	if(val == 1){
		...
		val = 1;
		if(isEmpty(&high) == 1 || isEmpty(&mid) == 1|| isEmpty(&low) == 1){
			task = getTask(&high, &mid, &low);
		}
		sem = resources.resources[task.resources[0]].semaphore;
		sem2 = resources.resources[task.resources[1]].semaphore;
